...
from ..utils import current_month_number, current_year
...
@click.option("--year", default=current_year(), prompt="Year to use")
@click.option(
    "--month",
...
def create_sheet_for_month(year, month, start, end, environment):
    ...
    balances = daily_balances_for_month(year, month, start, end, transactions)

    worksheet = _write_balances_to_spreadsheet(spreadsheet, balances, year, month)
